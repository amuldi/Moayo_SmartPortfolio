  ...
  return (
    <div
      ...
      className={clsx(
        ...
        hover && 'cursor-pointer hover:border-[var(--accent)] hover:shadow-lg hover:shadow-[var(--accent-soft)]',
        className
      )}
    >
      {children}
...
export function CardFooter({ children, className }) {
  return (
    <div className={clsx('px-5 py-3 border-t border-[var(--border)] bg-[var(--bg-elevated)] rounded-b-[var(--radius-card)]', className)}>
      {children}
    </div>
  )
}
    ...
    <Card className={clsx('p-5', accent && 'border-[var(--accent)] bg-[var(--accent-soft)]')}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-2xl font-semibold text-[var(--text-primary)] leading-tight truncate">
            {value}
          </p>
          {sub && (
