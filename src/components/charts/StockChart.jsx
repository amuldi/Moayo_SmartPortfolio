...
export function StockChart({ ticker, height = 280, initialQuote = null }) {
  ...
  const loadChart = useCallback(async () => {
    ...
    try {
      ...
      if (data.candles?.length > 0) {
        ...
        setQuoteError(data.error || '차트 데이터가 없어 추정 차트를 표시합니다.')
        setIsLive(false)
      }
    } catch {
      setCandles(getMockCandles(ticker, range))
      setQuoteError('차트 데이터를 불러오지 못해 추정 차트를 표시합니다.')
      setIsLive(false)
    } finally {
      setLoading(false)
    }
  ...
  const loadQuote = useCallback(async () => {
    try {
      ...
      setQuote(q)
      setLastUpdated(q.updatedAt ? new Date(q.updatedAt) : new Date())
      setQuoteError(q.errors?.[0] || '')
      setIsLive(!q.isEstimated && !q.stale)
    } catch (error) {
      setQuoteError(error.message || '시세를 불러오지 못했습니다.')
    }
  }, [ticker])
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 pt-4 pb-3 flex items-start justify-between gap-4">
        <div>
          ...
          {quoteError && (
            <p className="mt-1 text-[10px] text-[var(--text-muted)]">{quoteError}</p>
          )}
        </div>
