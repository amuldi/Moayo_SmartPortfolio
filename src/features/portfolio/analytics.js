...
function convertToKrw(amount, currency, fxRates = FX_RATES) {
  return amount * (fxRates[currency] || FX_RATES[currency] || 1)
}

export function buildHoldingMetrics(holding, account, livePrices = {}, fxRates = FX_RATES) {
  const quote = getQuoteForHolding(holding, livePrices)
  const asset = getAssetInfo(holding.ticker)
  const quantity = Number(holding.quantity || 0)
  const avgPrice = Number(holding.avgPrice || 0)
  const importedCurrentPrice = Number(holding.currentPrice || 0)
  const importedPurchaseAmount = Number(holding.purchaseAmount || 0)
  const importedMarketValue = Number(holding.marketValue || 0)
  const hasImportedValuation = importedPurchaseAmount > 0 || importedMarketValue > 0 || importedCurrentPrice > 0
  const currentPrice = importedCurrentPrice > 0 ? importedCurrentPrice : quote.price
  const quoteCurrency = importedCurrentPrice > 0 ? holding.currency : quote.currency
  const invested = importedPurchaseAmount > 0 ? importedPurchaseAmount : quantity * avgPrice
  const marketValue = importedMarketValue > 0 ? importedMarketValue : quantity * currentPrice
  const investedKRW = importedPurchaseAmount > 0 ? importedPurchaseAmount : convertToKrw(invested, holding.currency, fxRates)
  const marketValueKRW = importedMarketValue > 0 ? importedMarketValue : convertToKrw(marketValue, quoteCurrency, fxRates)
  const importedPnl = Number(holding.pnl || 0)
  const pnl = hasImportedValuation && (importedPnl || importedMarketValue > 0 || importedPurchaseAmount > 0)
    ? importedPnl || (marketValueKRW - investedKRW)
    : marketValueKRW - investedKRW
  const importedReturnPct = Number(holding.returnPct || 0)
  const pnlPct = hasImportedValuation && importedReturnPct
    ? importedReturnPct
    : investedKRW > 0 ? (pnl / investedKRW) * 100 : 0
  const previousClose = currentPrice / (1 + quote.changePct / 100)
  const dailyPnl = hasImportedValuation ? 0 : convertToKrw((currentPrice - previousClose) * quantity, quoteCurrency, fxRates)
  ...
  return {
    ...
    avgPrice,
    currentPrice,
    currentPriceKRW: importedCurrentPrice > 0 ? importedCurrentPrice : convertToKrw(currentPrice, quoteCurrency, fxRates),
    invested,
    investedKRW,
    marketValue,
    marketValueKRW,
...
function buildMonthlyTimeline(holdings, fxRates = FX_RATES) {
  ...
  holdings.forEach((holding) => {
    ...
    history.forEach((point) => {
      const value = convertToKrw(point.price * holding.quantity, holding.currency, fxRates)
...
function buildRebalancingSuggestions(holdings) {
  ...
  const totalValue = holdings.reduce((sum, holding) => sum + holding.marketValueKRW, 0)
  ...
  return candidates
    .map((holding) => ({
      ...
      action: holding.targetWeight - holding.weight > 0 ? '비중 확대 검토' : '비중 축소 검토',
      amount: (Math.abs(holding.targetWeight - holding.weight) / 100) * totalValue,
    }))
...
export function buildPortfolioSnapshot(accounts = [], livePrices = {}, fxRates = FX_RATES) {
  const enrichedAccounts = accounts.map((account) => {
    const holdings = account.holdings.map((holding) => buildHoldingMetrics(holding, account, livePrices, fxRates))
    const investedKRW = holdings.reduce((sum, holding) => sum + holding.investedKRW, 0)
    const holdingsMarketValueKRW = holdings.reduce((sum, holding) => sum + holding.marketValueKRW, 0)
    const marketValueKRW = Math.max(holdingsMarketValueKRW, Number(account.totalCapital || 0))
    const dailyPnl = holdings.reduce((sum, holding) => sum + holding.dailyPnl, 0)
    const hasImportedPnl = holdings.some((holding) => Number(holding.purchaseAmount || 0) > 0 || Number(holding.marketValue || 0) > 0 || Number(holding.returnPct || 0) !== 0)
    const pnl = hasImportedPnl
      ? holdings.reduce((sum, holding) => sum + holding.pnl, 0)
      : holdingsMarketValueKRW - investedKRW
    const cashBuffer = Math.max(marketValueKRW - holdingsMarketValueKRW, 0)
  ...
  const totalValueKRW = enrichedAccounts.reduce((sum, account) => sum + account.marketValueKRW, 0)
  const totalDailyPnl = holdings.reduce((sum, holding) => sum + holding.dailyPnl, 0)
  const totalPnl = enrichedAccounts.reduce((sum, account) => sum + account.pnl, 0)
  ...
  const timeline = buildMonthlyTimeline(weightedHoldings, fxRates)
