...
function analyzeRegionalDiversification(aggregatedHoldings) {
  ...
  if (koreaAlloc > 60) {
    insights.push({
      ...
      suggestion: '글로벌 ETF(VT, SPY, QQQ 등)를 연금·ISA 계좌에 분산 편입하는 방안을 검토해보세요',
    })
  }
...
function analyzeAssetClassBalance(aggregatedHoldings) {
  ...
  if (cashPct > 15) {
    insights.push({
      ...
      suggestion: '5~10% 이상의 초과 현금은 주식·채권 ETF로 단계적 편입하는 방안을 검토해보세요',
    })
  }
  ...
  targetHoldings.forEach((target) => {
    ...
    if (Math.abs(gap) > 5) {
      suggestions.push({
        ...
        suggestion: gap > 0
          ? `${target.name} 비중 확대 검토 — 목표 비중보다 ${gap.toFixed(1)}%p 부족합니다`
          : `${target.name} 비중 축소 검토 — 목표 비중보다 ${Math.abs(gap).toFixed(1)}%p 초과합니다`,
      })
    }
  })
...
function generateSummary(healthScore, highCount, taxScore) {
  ...
  if (healthScore >= 70) {
    return `포트폴리오 상태는 양호하나, ${highCount}개 항목에 주의가 필요합니다. 아래 참고 사항을 검토해보세요.`
  }
  if (healthScore >= 50) {
    return `몇 가지 개선을 통해 위험조정수익률과 세금 효율을 크게 높일 수 있습니다. 권장 조치를 확인하세요.`
  }
  return '집중도 또는 분산투자 측면에서 중요한 문제가 있습니다. 체계적인 리밸런싱 계획 수립을 검토해보세요.'
}
