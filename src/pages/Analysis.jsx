...
function EmptyState() {
  return (
    <Card className="p-8">
      <p className="text-lg font-semibold text-[var(--text-primary)]">리밸런싱을 계산하려면 현재 포트폴리오가 필요합니다.</p>
      <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
        보유 종목과 계좌를 입력하면 현재 구조 문제와 추천 포트폴리오, 조정 순서를 바로 제안합니다.
      </p>
...
export default function Analysis() {
  const { accounts, livePrices, currentPortfolioName, refreshStatus, refreshError, lastPriceUpdateAt } = usePortfolioStore()
  ...
  const snapshot = useMemo(() => buildPortfolioSnapshot(accounts, livePrices), [accounts, livePrices])
    ...
    ? [
        {
          ...
          desc: '집중도와 중복 노출이 높을 때 유리합니다. 다만 체감 변동성이 커질 수 있어 매도/매수 순서를 꼭 지키는 편이 좋습니다.',
        },
        {
          id: 'split',
          label: '분할 전환',
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-[var(--text-primary)]">리밸런싱</h1>
            <p className="mt-1 text-sm text-[var(--text-muted)]">
              현재 상태를 진단하고, 더 나은 구조와 실제 매수·매도 순서를 바로 제안합니다.
            </p>
            <p className="mt-2 text-xs text-[var(--text-secondary)]">
              {refreshError
                ? '실시간 시세 서버 연결에 실패해 기본 시세로 계산 중입니다. `npm run server` 실행 여부를 확인해 주세요.'
                : lastPriceUpdateAt
                  ? `${new Date(lastPriceUpdateAt).toLocaleString('ko-KR')} 기준 실시간 시세 반영`
          ...
          <>
            <Card>
              <CardBody className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--accent)]">Current → Target → Action</p>
                  <h2 className="mt-2 text-2xl font-bold text-[var(--text-primary)]">
                    현재 구조를 더 나은 포트폴리오로 바꾸는 조정 가이드
                  </h2>
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--text-secondary)]">
                    {currentPortfolioName}의 현재 문제를 먼저 짚고, 투자 성향에 맞는 목표 구조를 고른 뒤,
                    지금 계좌에서 무엇을 먼저 줄이고 무엇을 늘릴지 단계별로 정리했습니다.
                  </p>
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    {summaryCards.map((item) => (
                      <div key={item.label} className="flex h-full min-h-[126px] flex-col justify-between rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-elevated)] p-4">
                ...
                <div className="grid gap-3 xl:sticky xl:top-24">
                  ...
                  <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-elevated)] p-3">
                    <p className="text-xs font-semibold text-[var(--text-primary)]">
                      {INVESTOR_PROFILES.find((item) => item.value === profile)?.label}
                    </p>
                    <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">
                      {INVESTOR_PROFILES.find((item) => item.value === profile)?.desc}
                    </p>
                  </div>
                  <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-card)] p-4">
                    ...
                    <p className="mt-3 text-xs leading-5 text-[var(--text-secondary)]">{planning.portfolioScore.summary}</p>
            ...
            <div className="sticky top-0 z-10 -mx-4 border-y border-[var(--border)] bg-[var(--bg-primary)]/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6">
                  ...
                  <>
                    <Card>
                      ...
                      <CardBody className="grid gap-4 xl:grid-cols-[1fr_0.9fr]">
                        <div className="space-y-3">
                          {selectedRecommendation.majorChanges.map((change) => (
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div className="text-right">
                                  <p className={clsx('text-sm font-semibold', change.gap > 0 ? 'positive' : 'negative')}>
                                    {change.gap > 0 ? '늘리기' : '줄이기'} {Math.abs(change.gap).toFixed(1)}%p
                                  </p>
                                  <p className="mt-1 text-xs text-[var(--text-secondary)]">
                                    {change.gap > 0 ? '부족 비중이라 분할 매수 우선 후보입니다.' : '초과 비중이라 먼저 정리할 후보입니다.'}
                                  </p>
                                </div>
                              </div>
                            </div>
