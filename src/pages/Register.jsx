...
function GoogleRegisterUnavailableButton({ setError }) {
  return (
    <button
      type="button"
      onClick={() => setError('Google 가입을 사용하려면 VITE_GOOGLE_CLIENT_ID 환경변수를 등록해주세요.')}
...
function GoogleRegisterButton({ finishLogin, setError, setSocialLoading }) {
  return (
    <div className="flex w-full justify-center rounded-xl border bg-[var(--bg-card)] py-1" style={{ borderColor: 'var(--border)' }}>
      ...
      try {
        ...
        const data = await readApiJson(res, 'Google 가입 실패')
        await finishLogin(data.user)
      } catch (error) {
        setError(error.message || 'Google 가입 중 오류가 발생했습니다')
      } finally {
        setSocialLoading('')
      }
        }}
...
function NaverRegisterButton({ finishLogin, setError, socialLoading, setSocialLoading }) {
  const handleNaverLogin = async () => {
    ...
    try {
      ...
      setError(error.message || '네이버 가입 중 오류가 발생했습니다')
    } finally {
      setSocialLoading('')
    }
  }
...
function RegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await readApiJson(res, '회원가입에 실패했습니다')
      setDone(true)
    } catch (error) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col gap-2 mb-5">
          {GOOGLE_CLIENT_ID ? (
            ...
            <GoogleRegisterUnavailableButton setError={setError} />
          )}
          <NaverRegisterButton
            finishLogin={finishLogin}
            setError={setError}
